...
export function CreateGroupForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // If we get here, there was an error (redirect would have happened otherwise)
    if (result?.error) {
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Configurá cuántos puntos vale cada tipo de acierto.
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Definí qué se lleva cada puesto.
          </p>
        </div>
        ...
        {[
          ...
          <div key={name}>
            ...
            <input
              ...
              name={name}
              maxLength={100}
              placeholder="Ej: 1 botella de whisky"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A2342] focus:border-transparent"
            />
          </div>
        ))}
